...
def pick_band_asset(item, common_names: Iterable[str]) -> str:
    ...
    Avoids hard-coding Landsat band IDs across sensors.
    """
...
def build_year(catalog: Client, roi_gdf: gpd.GeoDataFrame, year: int) -> None:
    ...
    roi_geom = roi_gdf.geometry.iloc[0].__geo_interface__
    roi_bounds = tuple(roi_gdf.total_bounds)  # (minx, miny, maxx, maxy) EPSG:4326
    ...
    # Sign items so asset hrefs are readable (adds short-lived token if needed).
    ...
    stack = np.stack(ndsi_stack, axis=0)
